...
function makeResponse(response, config) {
  ...
  return {
    ...
    download(path$1, name) {
      ...
      _headers["Content-Disposition"] = "attachment; filename=" + (name || path.basename(path$1));
      throw this;
    },
    getHeaders() {
      return _headers;
    ...
    [__exec__]() {
      try {
        ...
        if (this.data)
          ...
          fs.createReadStream(this.file).pipe(response);
          return;
        }
        response.end();
      } catch (error) {
